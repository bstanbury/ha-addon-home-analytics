#!/usr/bin/env python3
"""Home Analytics Dashboard v3.0.0
Event-driven trend analysis with persistent daily summaries and anomaly detection.

v3.0 additions:
  - Track DJ playlist plays/skips as part of daily stats
  - Sleep disruption event tracking
...
Endpoints:
  ...
  GET /stats/dj            — DJ playlist play/skip tracking
  GET /stats/sleep-disruptions — Sleep disruption events
...
SPOTIFY_URL = os.environ.get('SPOTIFY_URL', 'http://localhost:8097')
...
# v3.0: DJ tracking
dj_plays = defaultdict(int)   # playlist_id -> play count today
dj_skips = defaultdict(int)   # playlist_id -> skip count today

# v3.0: Sleep disruption tracking
...
def flush_today():
    ...
    stats_db[today] = {
        ...
        # v3.0: DJ stats
        'dj_plays': dict(dj_plays),
        'dj_skips': dict(dj_skips),
...
def detect_anomaly(entity_id, value, label):
    ...
    for day_data in list(stats_db.values())[-30:]:
        ...
        if avg: historical.append(avg)
    if len(historical) < 7: return None
    mean = sum(historical) / len(historical)
    std_dev = (sum((x - mean) ** 2 for x in historical) / len(historical)) ** 0.5
    if std_dev == 0: return None
    z_score = abs(value - mean) / std_dev
    if z_score > 2.5:
        return {'entity': entity_id, 'label': label, 'value': value,
            'historical_mean': round(mean, 1), 'z_score': round(z_score, 2), 'time': datetime.now().isoformat()}
    return None
...
def handle_event(ev):
    ...
    # v3.0: Track sleep disruption events
    if classification == 'sleep_disrupting':
        sleep_disruptions.append({
            'time': datetime.now().isoformat(), 'entity_id': eid,
            'old': old, 'new': new, 'classification': classification,
        })
        action = 'sleep_disruption_tracked'
...
def poll_dj_stats():
    """v3.0: Periodically poll Spotify DJ for play/skip stats."""
    while True:
        try:
            r = http.get(f'{SPOTIFY_URL}/stats', timeout=5)
            if r.status_code == 200:
                data = r.json()
                for p in data.get('top_playlists', []):
                    pid = p.get('id', '')
                    if pid:
                        dj_plays[pid] = p.get('plays', 0)
                        dj_skips[pid] = p.get('skips', 0)
        except: pass
...
@app.route('/')
def index():
    return jsonify({'name': 'Home Analytics Dashboard', 'version': '3.0.0', 'days_tracked': len(stats_db), 'event_bus': 'connected' if event_actions else 'waiting'})
...
def energy_today():
    ...
    return jsonify({'heater_daily_kwh': float(heater_kwh['state']) if heater_kwh else 0,
...
def energy_week():
    ...
    return jsonify({'heater_estimated_weekly_kwh': round(daily * 7, 1), 'heater_estimated_weekly_cost': round(daily * 7 * 0.35, 2), 'rate_per_kwh': 0.35})
...
def sleep_last():
    ...
    # v3.0: Penalize sleep disruption events
    today = datetime.now().strftime('%Y-%m-%d')
...
def presence_today():
    ...
    events = []
    for e in h: events.append({'time': e['last_changed'], 'state': e['state']})
    home_mins = 0
    away_mins = 0
...
def presence_patterns():
    ...
    for day, data in sorted(stats_db.items())[-30:]:
        p = data.get('presence', {})
        if p: patterns.append({'date': day, 'arrivals': p.get('arrivals', 0), 'departures': p.get('departures', 0)})
...
def batteries():
    ...
    for s in states:
        if 'battery' in s['entity_id'] and s['entity_id'].startswith('sensor.') and s['state'] not in ['unavailable', 'unknown']:
            try:
                val = float(s['state'])
                if 0 <= val <= 100: batts.append({'entity': s['entity_id'], 'name': s['attributes'].get('friendly_name', s['entity_id']), 'level': val})
            except: pass
...
def summary():
    ...
    return jsonify({
        'home': presence['state'] == 'on' if presence else None, 'lock': lock['state'] if lock else None,
        'bedroom_temp': float(climate['state']) if climate else None, 'vacuum': vacuum['state'] if vacuum else None,
        'soil_moisture': float(soil['state']) if soil else None, 'days_tracked': len(stats_db),
        'arrivals_today': arrival_count, 'departures_today': departure_count, 'timestamp': datetime.now().isoformat()})
...
def stats_anomalies():
    anomalies_list = [e for e in event_actions if e.get('action') == 'anomaly_detected']
    return jsonify({'anomalies': anomalies_list[-20:], 'total': len(anomalies_list)})

# v3.0 endpoints
@app.route('/stats/dj')
def stats_dj():
    """v3.0: DJ playlist play/skip tracking."""
    return jsonify({'plays': dict(dj_plays), 'skips': dict(dj_skips), 'total_plays': sum(dj_plays.values()), 'total_skips': sum(dj_skips.values())})

@app.route('/stats/sleep-disruptions')
def stats_sleep_disruptions():
    """v3.0: Sleep disruption events."""
    return jsonify({'events': list(sleep_disruptions)[-30:], 'total': len(sleep_disruptions)})
...
if __name__ == '__main__':
    logger.info(f'Home Analytics Dashboard v3.0.0 on :{API_PORT}')
    load_stats_db()
    threading.Thread(target=event_bus_subscriber, daemon=True).start()
    threading.Thread(target=daily_flush_loop, daemon=True).start()
    threading.Thread(target=poll_dj_stats, daemon=True).start()
    logger.info('Event Bus subscriber + daily flush + DJ stats poller started')
